...
const ProductCardUpdater = {
    ...
    removeNigerianFlags: function(card) {
        ...
        card.querySelectorAll('.product-origin-badge, .origin-badge-nigerian, .nigeria-flag-colors-small').forEach(el => {
            el.remove();
        });

        // Remove any elements containing "Nigerian" text
        card.querySelectorAll('*').forEach(el => {
            if (el.textContent && el.textContent.toLowerCase().includes('nigerian') &&
                !el.classList.contains('product-title') &&
    ...
    overrideFilterFunction: function() {
        ...
        // Add event listener for DOM changes to catch dynamically added/modified elements
        this.observeDOM();
    },

    // Observe DOM changes to catch dynamically added/modified elements
    observeDOM: function() {
        // Create a MutationObserver to watch for changes to the DOM
        const observer = new MutationObserver((mutations) => {
            let shouldUpdate = false;

            // Check if any of the mutations involve product cards
            mutations.forEach(mutation => {
                ...
                // Check if the target is a product card or contains product cards
                if (!shouldUpdate && mutation.target.nodeType === 1) {
                    if (mutation.target.classList &&
                        (mutation.target.classList.contains('product-card') ||
                         mutation.target.classList.contains('product-item') ||
                         mutation.target.querySelector('.product-card, .product-item'))) {
                        shouldUpdate = true;
                    }
                }
            });

            // If we found product cards, update them
            if (shouldUpdate) {
                this.updateProductCards();
            }
        });
        ...
        observer.observe(document.body, {
            ...
            subtree: true,
            attributes: true,
            attributeFilter: ['class', 'style', 'data-category-id']
        });
    }
};
